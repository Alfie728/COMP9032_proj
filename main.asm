...
.equ size3 = 15
.equ l3 = size3 * size3
.equ MAP_SIZE           = 7                 ; maximum grid 15x15
.equ MAP_CELLS          = 49
...
VEditCnt:              .byte 1
	...
	; ----- Part 3: terrain + path data -----map:	.byte l3
map:	.byte l3
c_vis:	.byte l3
p_vis:	.byte l3
check:	.byte l3
...
GenerateSearchPath:
	...
	; Initialise working maps and observation buffers
	load_array_from_program m, map, l3
	load_array_from_program zeros, p_vis, l3
	load_array_from_program zeros, c_vis, l3
	load_array_from_program zeros, check, l3
	load_array_from_program zeros, max_map, l3
	load_array_from_program zeros, obs_x, l3
	load_array_from_program zeros, obs_y, l3

	; Run greedy search to populate obs_x/obs_y and obs_len
	greedy_search

	; Prepare pointers for copying into ObservationPath
	ldi YL, low(obs_x)
	ldi YH, high(obs_x)
	ldi XL, low(obs_y)
	ldi XH, high(obs_y)
	ldi ZL, low(ObservationPath)
	ldi ZH, high(ObservationPath)

	lds workC, obs_len
	cpi workC, MAX_OBS_POINTS
	brlo gsp_len_ok
	ldi workC, MAX_OBS_POINTS
gsp_len_ok:
	clr workB
	tst workC
	breq gsp_copy_done
...
gsp_copy_loop:
	; load next observation (x from obs_x, y from obs_y)
	ld workD, Y+
	ld workE, X+
	st Z+, workD
	st Z+, workE
	; preserve obs_y pointer across array_i_j call
	mov temp12, XL
	mov temp13, XH
	; fetch terrain height for this coordinate
	mov temp0, workD
	mov temp1, workE
	array_i_j map, size3, temp0, temp1
	ld workF, x
	st Z+, workF
	; restore obs_y pointer
	mov XL, temp12
	mov XH, temp13
	inc workB
	cp workB, workC
